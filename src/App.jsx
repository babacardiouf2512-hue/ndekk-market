...
const VENDORS = [
  { id: 1, name: "Fatou Couture", phone: "77 123 45 67", city: "Dakar", avatar: "👩🏾", bio: "Créatrice de mode depuis 2015. Spécialiste boubous et tenues traditionnelles.", verified: true, premium: true, sales: 234, rating: 4.9, joined: "Jan 2023" },
  { id: 2, name: "Tech Mbaye", phone: "76 987 65 43", city: "Dakar", avatar: "👨🏾‍💻", bio: "Revendeur officiel Samsung & Tecno. Garantie 6 mois sur tous les appareils.", verified: true, premium: true, sales: 87, rating: 4.7, joined: "Mar 2023" },
  { id: 3, name: "Maman Rokhaya", phone: "78 456 78 90", city: "Saint-Louis", avatar: "👩🏾‍🍳", bio: "Cuisine traditionnelle sénégalaise. Livraison possible dans Saint-Louis.", verified: false, premium: false, sales: 412, rating: 5.0, joined: "Juil 2023" },
  { id: 4, name: "Artisan Gaye", phone: "70 321 09 87", city: "Mbour", avatar: "👨🏾‍🎨", bio: "Artisan vannier depuis 20 ans. Pièces uniques faites à la main.", verified: true, premium: false, sales: 156, rating: 4.8, joined: "Avr 2023" },
];

const PRODUCTS = [
  { id: 1, name: "Boubou brodé Premium", price: 45000, vendorId: 1, category: "Mode", emoji: "👘", rating: 4.9, reviews: 28, boosted: true, boostType: "vedette", stock: 5 },
  { id: 2, name: "Thiébou dieune royal", price: 5500, vendorId: 3, category: "Alimentation", emoji: "🍚", rating: 5.0, reviews: 94, boosted: false, stock: 20 },
  { id: 3, name: "Samsung Galaxy A55", price: 185000, vendorId: 2, category: "Électronique", emoji: "📱", rating: 4.8, reviews: 12, boosted: true, boostType: "premium", stock: 3 },
  { id: 4, name: "Tissu wax Holland 6y", price: 12000, vendorId: 1, category: "Mode", emoji: "🎨", rating: 4.9, reviews: 67, boosted: false, stock: 15 },
  { id: 5, name: "Mangues Casamance 5kg", price: 3500, vendorId: 3, category: "Alimentation", emoji: "🥭", rating: 4.7, reviews: 140, boosted: false, stock: 30 },
  { id: 6, name: "Panier osier XL", price: 8500, vendorId: 4, category: "Maison", emoji: "🧺", rating: 4.8, reviews: 45, boosted: true, boostType: "vedette", stock: 8 },
  { id: 7, name: "Thiouraye luxe", price: 3200, vendorId: 4, category: "Maison", emoji: "🌿", rating: 4.9, reviews: 203, boosted: false, stock: 50 },
  { id: 8, name: "Chaussures cuir Dakar", price: 28000, vendorId: 1, category: "Mode", emoji: "👞", rating: 4.6, reviews: 33, boosted: false, stock: 7 },
];

const REVIEWS = [
  { id: 1, productId: 1, author: "Aminata S.", avatar: "👩🏾", rating: 5, text: "Qualité exceptionnelle, couture parfaite. Je recommande vivement !", date: "2 jours" },
  { id: 2, productId: 1, author: "Moussa D.", avatar: "👨🏾", rating: 5, text: "Magnifique boubou, livré rapidement. Vendeur sérieux.", date: "1 semaine" },
  { id: 3, productId: 3, author: "Ibrahima F.", avatar: "👨🏾‍💼", rating: 4, text: "Téléphone comme décrit, garantie respectée.", date: "3 jours" },
  { id: 4, productId: 2, author: "Ndéye M.", avatar: "👩🏾‍🦱", rating: 5, text: "Le meilleur thiébou de Saint-Louis ! Ma famille adore.", date: "5 jours" },
];
...
export default function NdekkMarket() {
  ...
  const [boostHistory, setBoostHistory] = useState([
    { vendor: "Fatou Couture", plan: "⭐ Vedette", amount: 3000, date: "08 Avr" },
    { vendor: "Tech Mbaye", plan: "💎 Premium", amount: 12000, date: "05 Avr" },
  ]);
  ...
  const reviewsOf = (pid) => REVIEWS.filter(r => r.productId === pid);
  ...
  const handleAuth = () => {
    if (authMode === "register") {
      ...
      const nv = { id: vendors.length + 1, name: authForm.name, phone: authForm.phone, city: authForm.city, avatar: authForm.avatar, bio: "Nouveau vendeur sur Ndëkk Market.", verified: false, premium: false, sales: 0, rating: 5.0, joined: "Avr 2026" };
      setVendors(v => [...v, nv]);
      setCurrentUser(nv);
      showToast(`🎉 Bienvenue ${authForm.name} !`);
      setView("vendeur-space");
    } else {
      const found = vendors.find(v => v.phone === authForm.phone);
      if (found) { setCurrentUser(found); showToast(`👋 Content de te revoir, ${found.name} !`); setView("vendeur-space"); }
  ...
  const postProduct = () => {
    ...
    setProducts(prev => [{ id: prev.length + 1, name: newProduct.name, price: parseInt(newProduct.price), vendorId: currentUser?.id || 1, category: newProduct.category, emoji: newProduct.emoji || "📦", rating: 5.0, reviews: 0, boosted: false, stock: parseInt(newProduct.stock) || 10 }, ...prev]);
    setNewProduct({ name: "", price: "", category: "Mode", emoji: "📦", stock: "" });
    showToast("🚀 Produit publié !");
  };

  // Styles globaux — overflow fix appliqué partout
  const G = {
    ...
    <div style={G.wrap}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cat-pill { flex-shrink: 0; padding: 6px 14px; border-radius: 8px; border: 1px solid rgba(201,168,76,0.2); background: #111; cursor: pointer; font-size: 0.78rem; font-weight: 500; color: #A09080; transition: all 0.2s; white-space: nowrap; }
        ...
        .pcard-vendor { font-size: 0.68rem; color: #6A5A4A; margin-bottom: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .sec-sub { font-size: 0.72rem; color: #6A5A4A; letter-spacing: 0.1em; text-transform: uppercase; }
        ...
        .btn-ghost { background: none; border: 1px solid rgba(201,168,76,0.2); color: #A09080; padding: 10px 18px; border-radius: 9px; font-weight: 500; font-size: 0.82rem; cursor: pointer; }
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 52 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {currentUser
              ...
              : <button className="btn-ghost" style={{ padding: "6px 12px", fontSize: "0.75rem" }} onClick={() => { setAuthMode("register"); setView("auth"); }}>Vendre</button>
            }
      ...
      {view === "home" && (
        <div style={{ overflowX: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", borderTop: "1px solid rgba(201,168,76,0.1)", borderBottom: "1px solid rgba(201,168,76,0.1)" }}>
            {[["1 240", "Vendeurs"], ["14", "Villes"], ["98%", "Satisfaction"], ["Wave", "Paiement"]].map(([v, l]) => (
          ...
          <div style={G.page}>
                ...
                return (
                  <div className="pcard boosted" key={p.id} onClick={() => { setSelectedProduct(p); setView("product"); }}>
                    <div className="pcard-thumb">
                      <span>{p.emoji}</span>
                      <span className="pcard-badge" style={{ background: p.boostType === "premium" ? "#A78BFA" : "#C9A84C", color: "#0A0A0A" }}>
                        {p.boostType === "premium" ? "💎" : "⭐"}
                      </span>
                    </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", margin: "28px 0 14px" }}>
              <div className="sec-title" style={{ marginBottom: 0 }}>Vendeurs de confiance</div>
              ...
              <div className={`vcard ${v.premium ? "premium" : ""}`} key={v.id} onClick={() => { setSelectedVendor(v); setView("vendor-profile"); }}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "0.78rem", color: "#A09080", lineHeight: 1.5, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{v.bio}</div>
                    <div style={{ display: "flex", gap: 16, marginTop: 8 }}>
                      {[[ v.sales, "ventes"], [`⭐ ${v.rating}`, "note"], [products.filter(p => p.vendorId === v.id).length, "produits"]].map(([val, lab]) => (
                        <div key={lab} style={{ fontSize: "0.72rem", color: "#6A5A4A" }}><strong style={{ color: "#A09080", display: "block", fontSize: "0.82rem" }}>{val}</strong>{lab}</div>
                      ))}
                    </div>
                  </div>
                </div>
      ...
      {view === "market" && (
        <div style={G.page}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
            ...
            <span className="sec-sub">{filtered.length} résultats</span>
          </div>
            ...
            {filtered.map(p => {
              ...
              return (
                <div className={`pcard ${p.boosted ? "boosted" : ""}`} key={p.id} onClick={() => { setSelectedProduct(p); setView("product"); }}>
                  <div className="pcard-thumb">
                    <span>{p.emoji}</span>
                    {p.boosted && <span className="pcard-badge" style={{ background: p.boostType === "premium" ? "#A78BFA" : "#C9A84C", color: "#0A0A0A" }}>{p.boostType === "premium" ? "💎" : "⭐"}</span>}
                  </div>
                  <div className="pcard-body">
                    <div className="pcard-vendor">{v?.avatar} {v?.name}</div>
                    <div className="pcard-name">{p.name}</div>
            ...
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", gap: 20, marginTop: 12 }}>
                  {[[selectedVendor.sales, "Ventes"], [`⭐ ${selectedVendor.rating}`, "Note"], [products.filter(p => p.vendorId === selectedVendor.id).length, "Produits"]].map(([v, l]) => (
                    <div key={l}><div style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "1.4rem", color: "#C9A84C", fontWeight: 700 }}>{v}</div><div style={{ fontSize: "0.65rem", color: "#6A5A4A", textTransform: "uppercase", letterSpacing: "0.07em" }}>{l}</div></div>
                  ))}
                </div>
              </div>
            </div>
      ...
      {view === "product" && selectedProduct && (
        <div style={G.page}>
          ...
          <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, height: 200, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "6rem", marginBottom: 18 }}>
            {selectedProduct.emoji}
          </div>
          {selectedProduct.boosted && <div style={{ marginBottom: 10 }}><Badge>{selectedProduct.boostType === "premium" ? "💎 Vendeur Premium" : "⭐ Produit Vedette"}</Badge></div>}
          <h1 style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "1.8rem", fontWeight: 700, color: "#F5F0E8", marginBottom: 6, lineHeight: 1.15 }}>{selectedProduct.name}</h1>
          <div style={{ marginBottom: 8 }}><Stars rating={selectedProduct.rating} /><span style={{ fontSize: "0.72rem", color: "#6A5A4A", marginLeft: 5 }}>{selectedProduct.reviews} avis</span></div>
          <div style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "2rem", color: "#C9A84C", fontWeight: 700, marginBottom: 16 }}>{selectedProduct.price.toLocaleString("fr-FR")} FCFA</div>
          {(() => { const v = vendorOf(selectedProduct.vendorId); return (
            <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#1A1A1A", borderRadius: 10, padding: "11px 13px", cursor: "pointer", marginBottom: 14 }} onClick={() => { setSelectedVendor(v); setView("vendor-profile"); }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: "0.7rem", color: "#6A5A4A" }}>📍 {v?.city} · {v?.sales} ventes</div>
              </div>
              {v?.verified && <Badge>✓</Badge>}
              <span style={{ color: "#6A5A4A" }}>→</span>
            </div>
              ...
              : reviewsOf(selectedProduct.id).map(r => (
                <div className="review-card" key={r.id}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                    ...
                    <span style={{ fontSize: "0.68rem", color: "#6A5A4A", marginLeft: "auto" }}>il y a {r.date}</span>
                  </div>
      ...
      {view === "vendeur-space" && (
        <div style={G.page}>
          ...
          <p style={{ fontSize: "0.78rem", color: "#6A5A4A", marginBottom: 14 }}>Paiement sécurisé via Wave 🌊</p>
          {BOOSTS.map(b => (
            <div className="boost-card" key={b.id} style={{ borderColor: b.color + "33", background: b.color + "08" }} onClick={() => setShowBoostModal(b)}>
              ...
              <button style={{ width: "100%", padding: "8px", background: b.color, color: "#0A0A0A", border: "none", borderRadius: 8, fontWeight: 700, fontSize: "0.82rem", cursor: "pointer" }}>
                🌊 Payer via Wave
              </button>
            </div>
          ))}
      ...
      {view === "admin" && (
        <div style={G.page}>
          ...
          <div className="kpi-row">
            {[[`${revenue.toLocaleString("fr-FR")} F`, "Revenus boosts"], [products.filter(p => p.boosted).length, "Boostés"], [products.length, "Produits"], [vendors.length, "Vendeurs"]].map(([v, l]) => (
          ...
          <div className="sec-title" style={{ fontSize: "1.15rem", marginBottom: 11 }}>💰 Paiements reçus</div>
          <div style={{ background: "#111", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1fr 1fr", padding: "9px 13px", background: "#1A1A1A", fontSize: "0.66rem", fontWeight: 600, color: "#6A5A4A", textTransform: "uppercase", letterSpacing: "0.07em" }}>
        ...
        <div className="overlay" onClick={() => setShowBoostModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ background: "rgba(0,102,204,0.08)", border: "1px solid rgba(0,102,204,0.2)", borderRadius: 12, padding: 16, textAlign: "center", margin: "0 0 14px" }}>
              ...
              <div style={{ fontSize: "0.68rem", color: "#6A5A4A", marginTop: 2 }}>{showBoostModal.label} · par {showBoostModal.period}</div>
            </div>
            <p style={{ fontSize: "0.78rem", color: "#6A5A4A", textAlign: "center", marginBottom: 4 }}>Tu seras redirigé vers l'app Wave pour confirmer.</p>
              ...
              : <>
                {cart.map((item, i) => (
                  <div className="cart-item" key={i}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: "0.86rem", color: "#F5F0E8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.name}</div>
                      <div style={{ color: "#C9A84C", fontWeight: 600, fontSize: "0.86rem" }}>{item.price.toLocaleString("fr-FR")} FCFA</div>
                    </div>
                    <button onClick={() => setCart(c => c.filter((_, j) => j !== i))} style={{ background: "none", border: "none", color: "#6A5A4A", cursor: "pointer", fontSize: "1rem", padding: "0 4px" }}>✕</button>
                  </div>
                ))}
